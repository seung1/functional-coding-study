import { defineConfig } from "vitepress";

// https://vitepress.dev/reference/site-config
export default defineConfig({
  title: "Functional Coding",
  description: "『함수형 코딩』 을 공부하고 정리한 학습 노트 (2025년 8월~)",
  lastUpdated: true,
  sitemap: {
    hostname: "https://functional-coding-study.wonhub.dev",
  },
  themeConfig: {
    // https://vitepress.dev/reference/default-theme-config
    nav: [
      { text: "홈", link: "/" },
      { text: "공부 기록", link: "/0" },
    ],
    logo: {
      src: "/images/logo.webp",
      alt: "Functional Coding Logo",
    },
    editLink: {
      pattern:
        "https://github.com/seung1/functional-coding-study/edit/main/docs/:path",
      text: "GitHub에서 수정하기",
    },
    search: {
      provider: "local",
      options: {
        translations: {
          button: {
            buttonText: "검색",
            buttonAriaLabel: "검색",
          },
          modal: {
            backButtonTitle: "뒤로가기",
            displayDetails: "더보기",
            noResultsText: "검색 결과를 찾지 못했어요.",
            resetButtonTitle: "모두 지우기",
            footer: {
              closeKeyAriaLabel: "닫기",
              closeText: "닫기",
              navigateDownKeyAriaLabel: "아래로",
              navigateText: "이동",
              navigateUpKeyAriaLabel: "위로",
              selectKeyAriaLabel: "선택",
              selectText: "선택",
            },
          },
        },
      },
    },
    outline: {
      label: "페이지 내용",
      level: [2, 3],
    },
    docFooter: {
      prev: "이전 페이지",
      next: "다음 페이지",
    },
    lastUpdated: {
      text: "마지막 업데이트",
      formatOptions: {
        dateStyle: "full",
      },
    },

    sidebar: [
      {
        text: "함수형 프로그래밍",
        link: "/0",
        items: [
          {
            text: "01. 쏙쏙 들어오는 함수형 코딩에 오신 것을 환영합니다",
            link: "/1",
          },
          { text: "02. 현실에서의 함수형 사고", link: "/2" },
        ],
      },
      {
        text: "Part 1: 액션과 계산, 데이터",
        collapsed: false,
        items: [
          { text: "03. 액션과 계산, 데이터의 차이를 알기", link: "/3" },
          { text: "04. 액션에서 계산 빼내기", link: "/4" },
          { text: "05. 더 좋은 액션 만들기", link: "/5" },
          {
            text: "06. 변경 가능한 데이터 구조를 가진 언어에서 불변성 유지하기",
            link: "/6",
          },
          {
            text: "07. 신뢰할 수 없는 코드를 쓰면서 불변성 지키기",
            link: "/7",
          },
          { text: "08. 계층형 설계 I", link: "/8" },
          { text: "09. 계층형 설계 II", link: "/9" },
        ],
      },
      {
        text: "Part 2: 일급 추상",
        collapsed: false,
        items: [
          { text: "10. 일급 함수 I", link: "/10" },
          { text: "11. 일급 함수 II", link: "/11" },
          { text: "12. 함수형 반복", link: "/12" },
          { text: "13. 함수형 도구 체이닝", link: "/13" },
          { text: "14. 중첩된 데이터에 함수형 도구 사용하기", link: "/14" },
          { text: "15. 타임라인 격리하기", link: "/15" },
          { text: "16. 타임라인 사이에 자원 공유하기", link: "/16" },
          { text: "17. 타임라인 조율하기" },
          { text: "18. 반응형 아키텍처와 어니언 아키텍처" },
          { text: "19. 함수형 프로그래밍 여행에 앞서" },
        ],
      },
    ],

    socialLinks: [
      {
        icon: "github",
        link: "https://github.com/seung1/functional-coding-study",
      },
    ],
  },

  head: [
    [
      "link",
      { rel: "icon", type: "image/x-icon", href: "/images/favicon.ico" },
    ],
  ],
});
